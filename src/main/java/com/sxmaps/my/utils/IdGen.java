...
public class IdGen {
	...
	public synchronized String getSn() {
		...
		//3,workerId << workerIdShift 机器节点左移12位
		//利用|的特性，把数据拼接起来
		return DateFormatUtils.format(timestamp, "yyyyMMddHHMMssSSS")+((datacenterId << datacenterIdShift) | (workerId << workerIdShift) | sequence);
	}
	...
	public static void main(String[] args) {
		...
		for(int i = 0 ;i<5;i++){
//			if((1L << i -1)==(-1L ^ (-1L << i))){
//				System.out.println(((1L << 5) -1));
//				System.out.println((-1L ^ (-1L << 5)));

				System.out.println(((1L << i) -1));
				System.out.println(-1L ^ (-1L << i));
//			}
